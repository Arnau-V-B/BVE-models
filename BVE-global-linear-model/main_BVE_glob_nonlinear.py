...
This version of the program is an adaptation of main_BVE_glob.py program so that it can be initialized
with an arbitrari streamfunction perturbation and a mean zonal flow. This way the results can be directly
compared to the main_BVE_glob_linear.py simulation.
...
def compute_adv(u, v, vort):
	...
	zeta_spec = grid2spec(vort, gridtype)
	...
	zeta_lambda_spec = lambda_derivative(zeta_spec)
...
def compute_vel(stream_spec):
	...
	(output) -->
		...
		v_spec      : 3D array (2, lmax+1, lmax+1) with the spectral coefficients of the meridional velocity
		              field multiplied by cos(latitude)
	"""
...
if __name__ == '__main__':
	...
	mean_vorticities = []			# mean vorticity
	...
	Z_spec = grid2spec(Z, gridtype)
	Z_theta_spec = theta_derivative(Z_spec)
	Z_theta = spec2grid(Z_theta_spec, gridtype) * derfact
	...
	adv0_spec = compute_adv(u0, v0, zeta0)
	...
	evo.attrs = {
		'description': 'Evolution of 500 hPa relative vorticity and streamfunction perturbation fields through BVE simulation',
		'Conventions': 'DF-1.7',
