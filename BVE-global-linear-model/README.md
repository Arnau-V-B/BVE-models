...
This model solves the non-divergent BVE on the sphere using spherical harmonics transforms. By setting the flag `MODE` to **linear** in the `config.py` file, the model will solve the equation linearized over a mean zonal flow $$\overline{U}$$ (i.e., only mean flow - perturbation interactions are simulated). Otherwise, if `MODE` is set to **nonlinear**, the full equation including the nonlinear terms will be solved (i.e., both mean flow - perturbation and perturbation - perturbation interactions are simulated). It is designed to simulate simple atmospheric dynamics under multiple idealized circulation scenarios and compare the effects of including or not the nonlinearity of the equations, so they can serve as toy models for quick testing. The workflow of the code is based on [technical documentation](https://www.gfdl.noaa.gov/wp-content/uploads/files/user_files/pjp/barotropic.pdf) from the NOAA Geophysical Fluid Dynamics Laboratory (GFDL).
...
Given a background flow with the following properties: $$u = \overline{U} + u' \ ; \ v = v'$$ (where the overline denotes the mean state and the prime the perturbation), the non-divergent BVE can be linearized as follows:
...
which is the equation solved when `MODE = 'linear'`. However, if all crossed products are considered, the non-divergent BVE becomes:
...
where the new term that appears is nonlinear in the sense that there are products between perturbations. This is the equation solved in when `MODE = 'nonlinear'`.
...
To prevent the simulation from being contaminated with aliasing patterns from interactions between the highest modes (i.e. the smallest waves) resulting from the products between fields, an extra term has to be added to the BVE:

$$\frac{\partial \zeta}{\partial t} = - \vec{v}\cdot\vec{\nabla}(\zeta + f) - \eta \nabla^4 \zeta \quad \mathrm{; with} \quad \eta = \frac{R^4}{\tau (l_{max}(l_{max} + 1))^2};$$

where $$R$$ is the Earth radius, $$\tau$$ the decay rate of the target waves and $$l_{max}$$ the highest wavenumber. This is known as the hyperdiffusion term and only dissipates the response at the smallest unresolved scales, so that they disappear soon after they are formed and also transfer of eddy energy from large to small scales is partially simulated. By default, the decay rate is set to 3 hours, but it can be increased for better resolution or decreased for even more smoothing.
